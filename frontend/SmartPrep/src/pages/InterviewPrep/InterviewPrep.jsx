...
const InterviewPrep = () => {
  ...
  const generateConceptExplanation = async (question)=>{
    try {
      ...
      setExplanation(null)
      setErrorMsg("Failed To Generate Questions, Try Again Later")
      console.error("Error: ",error);
    }finally{
      setIsLoading(false)
    }
  ...
  const toogleQuestionsPinStatus = async (questionId)=>{
    try {
  ...
  return (
    <DashboardLayout>
      ...
      <div className=" container mx-auto pt-4 pb-4 px-4 md:px-0">
            ...
            <AnimatePresence>
              {sessionData?.questions?.map((data, index)=>{
                return (
                  ...
                  layout //key prop to animate position changes 
                  layoutId={`question-${data._id || index}`} // help frames track specific items
                  >
                    <>
                    <QuestionCard
                      ...
                      onTogglePin = {()=>{toogleQuestionsPinStatus(data._id)}}
                    />
        ...
        >
          {errorMsg && (
            <p className='flex gap-2 text-sm text-amber-800 font-medium'>
